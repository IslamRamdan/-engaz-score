import{j as e}from"./app-BOs5s3re.js";import{A as h}from"./AppLayout-CNaUTkmj.js";import"./ThemeToggle-BGjED9ty.js";const b=({customer:t,group:a,jobs:d=[]})=>{const i={e_visa_number:a.customers?.[0]?.pivot?.e_number||null,request_date:(()=>{const s=new Date;s.setDate(s.getDate()-5);const n=s.getFullYear(),l=String(s.getMonth()+1).padStart(2,"0"),o=String(s.getDate()).padStart(2,"0");return`${n}/${l}/${o}`})(),passport_id:t.passport_number,passport_type:"عادي",gender:t.gender==="female"?"أنثى":"ذكر",passport_expiry:t.passport_expiry_date?.toISOString().split("T")[0]||"غير متوفر",embassy_title:a.visa?.consulate||"غير متوفر",visa_period:{work_temp_hajj_umrah:"تأشيرة العمل المؤقت لخدمات الحج والعمرة",work:"عمل",temporary_work:"عمل مؤقت"}[a.visa?.type||""]||"غير متوفر",entries_count:a.visa?.type==="temporary_work"?"سفرة واحدة - 365 يوم":"سفرة واحدة - 90 يوم",sponsor_name:a.visa?.sponsor?.name,name_ar:t.name_ar,name_en_mrz:(()=>{const s=(t.name_en||"").trim().split(/\s+/);if(s.length===0||s[0]==="")return"غير متوفر";const n=s[0]||"",l=s[1]||"",o=s[2]||"",m=s.length>1?s[s.length-1]:"";return[n,l,o,m].filter((r,c,x)=>r!==""&&(c!==3||x.indexOf(r)===c||s.length>3)).join(" ")})(),job:d.find(s=>s.Value===a.notes)?.Text||a.notes||"---",date_birth:t.birth_date?.toISOString().split("T")[0]||"غير متوفر",governorate_live:t.governorate||"غير متوفر",nationality:"مصر",purpose:{work:`عمل لدى ${a.visa?.sponsor?.name||"---"}`,temporary_work:`عامل مؤقت لدى ${a.visa?.sponsor?.name||"---"}`,work_temp_hajj_umrah:`عمل موسمي لدى ${a.visa?.sponsor?.name||"---"}`}[a.visa?.type||""]||"غير متوفر"};return e.jsxs(h,{children:[e.jsx("button",{id:"printPage",className:"print-btn",onClick:()=>window.print(),children:"طباعة طلب دخول"}),e.jsx("script",{dangerouslySetInnerHTML:{__html:`

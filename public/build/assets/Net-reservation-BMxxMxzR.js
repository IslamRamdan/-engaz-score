import{j as e}from"./app-WOQAtObt.js";import{A as p}from"./AppLayout-Do_QdNAy.js";import"./ThemeToggle-C-2XL7nH.js";function m(a){if(!a)return"غير متوفر";const t=a.split("T")[0];if(/^\d{4}-\d{2}-\d{2}$/.test(t))return t;const n=new Date(a);return isNaN(n.getTime())?"غير متوفر":n.toISOString().split("T")[0]}const b=({customer:a,group:t,jobs:n=[]})=>{const i={e_visa_number:t.customers?.[0]?.pivot?.e_number||null,request_date:(()=>{const s=new Date;s.setDate(s.getDate()-5);const l=s.getFullYear(),o=String(s.getMonth()+1).padStart(2,"0"),r=String(s.getDate()).padStart(2,"0");return`${l}/${o}/${r}`})(),passport_id:a.passport_number,passport_type:"عادي",gender:a.gender==="female"?"أنثى":"ذكر",passport_expiry:m(a.passport_expiry_date),embassy_title:t.visa?.consulate||"غير متوفر",visa_period:{work_temp_hajj_umrah:"تأشيرة العمل المؤقت لخدمات الحج والعمرة",work:"عمل",temporary_work:"عمل مؤقت"}[t.visa?.type||""]||"غير متوفر",entries_count:t.visa?.type==="temporary_work"?"سفرة واحدة - 365 يوم":"سفرة واحدة - 90 يوم",sponsor_name:t.visa?.sponsor?.name,name_ar:a.name_ar,name_en_mrz:(()=>{const s=(a.name_en||"").trim().split(/\s+/);if(s.length===0||s[0]==="")return"غير متوفر";const l=s[0]||"",o=s[1]||"",r=s[2]||"",x=s.length>1?s[s.length-1]:"";return[l,o,r,x].filter((c,d,h)=>c!==""&&(d!==3||h.indexOf(c)===d||s.length>3)).join(" ")})(),job:n.find(s=>s.Value===t.notes)?.Text||t.notes||"---",date_birth:m(a.birth_date),governorate_live:a.governorate||"غير متوفر",nationality:a.nationality||"غير متوفر",purpose:{work:`عمل لدى ${t.visa?.sponsor?.name||"---"}`,temporary_work:`عامل مؤقت لدى ${t.visa?.sponsor?.name||"---"}`,work_temp_hajj_umrah:`عمل موسمي لدى ${t.visa?.sponsor?.name||"---"}`}[t.visa?.type||""]||"غير متوفر"};return e.jsxs(p,{children:[e.jsx("button",{id:"printPage",className:"print-btn",onClick:()=>window.print(),children:"طباعة طلب دخول"}),e.jsx("script",{dangerouslySetInnerHTML:{__html:`
